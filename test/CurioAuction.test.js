...
const CurioAuctionTest = artifacts.require(
  "./CurioAuctionTest.sol"
);

// Contract module
contract("CurioAuction", function(accounts) {
  ...
  const deploy = async function(cut = 0) {
    ...
    await nft.createToken({ from: owner });
    await nft.createToken({ from: user1 });

    tokenId1 = await nft.tokensOfOwnerByIndex(owner, 0);
    tokenId2 = await nft.tokensOfOwnerByIndex(user1, 0);
  };
  ...
  describe("Create auction", function() {
    ...
    it("should fail to create auction for duration too large", async function() {
      ...
      await util.expectThrow(
        nft.createAuction(tokenId1, 100, 200, duration, { from: owner })
      );
    });
    ...
    it("should fail to create auction for nonexistant NFT", async function() {
      ...
      await util.expectThrow(
        nft.createAuction(noSuchTokenId, 100, 200, 60, { from: owner })
      );
    });
    ...
    it("should fail to create auction for NFT already on auction", async function() {
      ...
      await util.expectThrow(
        nft.createAuction(tokenId1, 100, 200, 60, { from: owner })
      );
    });
  ...
  describe("Bidding", function() {
    ...
    it("should fail to bid if auction has been concluded", async function() {
      ...
      await util.expectThrow(auction.bid(tokenId1, { from: user1, value: 101 }));
    });
    ...
    it("should be able to bid", async function() {
      ...
      const token1Owner = await nft.ownerOf(tokenId1);
    ...
    it("should fail to bid after someone else has bid", async function() {
      ...
      await util.expectThrow(auction.bid(tokenId1, { from: user2, value: 101 }));
    });
  ...
  describe("Owner cut", function() {
    ...
    it("should add owner's cut of sale to balance", async function() {
      ...
      await nft.createAuction(tokenId2, 100, 200, 60, { from: user1 });
      ...
      const token2Owner = await nft.ownerOf(tokenId2);
  ...
  describe("Cancel auctions while paused", function() {
    beforeEach(async function() {
      ...
      await nft.createAuction(tokenId2, 100, 200, 1000, { from: user1 });
    });

    it("should fail to cancel auction when not paused", async function() {
      await util.expectThrow(auction.cancelAuctionWhenPaused(tokenId2, { from: owner }));
    });

    it("should fail to cancel auction when not owner", async function() {
      await auction.pause({ from: owner });

      await util.expectThrow(auction.cancelAuctionWhenPaused(tokenId2, { from: user2 }));
    });
    ...
    it("should be able to cancel auction as owner when paused", async function() {
      ...
      const tokenId2Owner = await nft.ownerOf(tokenId2);
  ...
  describe("Current price computation", function() {
    // Helpers
    ...
    before(async function() {
      ...
      currentPrice = auction._calculateCurrentPricePublic;
    });
